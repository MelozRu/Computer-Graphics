...
namespace lab1
{
    ...
    class IntensityFilter : Filters
    {
        protected override Color calculateNewPixelColor(Bitmap image, int x, int y)
        {
            Color source = image.GetPixel(x, y);
            int k = 25;
            Color result = Color.FromArgb(Clamp(source.R+k,0,255), Clamp(source.G + k, 0, 255), Clamp(source.B + k, 0, 255));
            return result;
        }
    }
    class MatrixFilter : Filters
    {
        ...
        protected override Color calculateNewPixelColor(Bitmap image, int x, int y)
        {
            ...
            return Color.FromArgb(Clamp((int)resR, 0, 255),
    ...
    class SepiaFilter : Filters
    {
        protected override Color calculateNewPixelColor(Bitmap image, int x, int y)
        {
            ...
            int G = (int)(0.587 * image.GetPixel(x, y).G);
            int B = (int)(0.114 * image.GetPixel(x, y).B);
            int intensity = Clamp(R + G + B, 0, 255);
            int k = 35;
            return Color.FromArgb(Clamp(intensity + 2 * k, 0, 255), Clamp((int)(intensity + 0.5 * k), 0, 255), Clamp(intensity - k, 0, 255));
        }
    }
        ...
        public override Bitmap processImage(Bitmap image, BackgroundWorker worker)
        {
            ...
            for (int i = 0; i < result.Width; i++)
            {
                ...
                for (int j = 0; j < result.Height; j++)
                {
                    ...
                    g1 *= g1;  g2 *= g2;
                    b1 *= b1;  b2 *= b2;
                    int ResR = (int)Math.Sqrt(r1 + r2), ResG = (int)Math.Sqrt(g1 + g2), ResB = (int)Math.Sqrt(b1 + b2);
                    result.SetPixel(i, j, Color.FromArgb(Clamp((int)ResR, 0, 255),
                                  Clamp((int)ResG, 0, 255),
                                  Clamp((int)ResB, 0, 255)));
                }
            }
    ...
    class GreyWorldFilter : Filters
    {
        ...
        protected override Color calculateNewPixelColor(Bitmap image, int i, int j)
        {
            ...
            return Color.FromArgb(Clamp(resR, 0, 255),
                 Clamp(resG, 0, 255),
                 Clamp(resB, 0, 255));
        }
    }
    ...
    class ErosionFilter : MathMorfology
    {
        ...
        public ErosionFilter(int[,] _mask_)
        {
            this.mask = _mask_;
            standartColor = Color.FromArgb(255, 255, 255);
        }
    ...
    class ClosingFilter : MathMorfology
    {
        ...
        public override Bitmap processImage(Bitmap image, BackgroundWorker worker)
        {
            MathMorfology Erosion = new ErosionFilter(mask), Dilation = new DilationFilter(mask);
            Dilation.setAllCols(2 * image.Width);
            Erosion.setAllCols(2 * image.Width);
            Erosion.setProcessedCols(image.Width);
            Bitmap res = Dilation.processImage(image, worker);
            res = Erosion.processImage(res, worker);
            return res;
        }
    }
        ...
        public override Bitmap processImage(Bitmap image, BackgroundWorker worker)
        {
            ...
            for (int i = 0; i < res.Width; i++)
            {
                ...
                for (int j = 0; j < res.Height; j++)
                {
                    ...
                    res.SetPixel(i, j, Color.FromArgb(Clamp(R, 0, 255), Clamp(G, 0, 255), Clamp(B, 0, 255)));
                }
            }
            return res;
        }
